...
import { messengerUrl } from "@/lib/util";
...
export function FeedCard({ feed }: { feed: FeedCardData }) {
  const initial = feed.title.trim().charAt(0).toUpperCase() || "F";
  const href = `/feed/${encodeURIComponent(feed.id)}`;
  return (
    <div className="group relative flex flex-col rounded-2xl border border-ink-100 bg-white p-5 shadow-card transition hover:-translate-y-0.5 hover:shadow-cardHover">
      <Link href={href} className="absolute inset-0 z-0" aria-label={feed.title} />
      ...
      <p className="mt-4 line-clamp-3 min-h-[3.6rem] text-sm leading-relaxed text-ink-500">
        {feed.previewText || "No recent posts."}
      </p>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-ink-100 pt-3">
        ...
        {feed.username && (
          <a
            href={messengerUrl(feed.username)}
            target="_blank"
